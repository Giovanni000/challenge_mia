# AN2DL25 — Challenge 1: Multivariate Time Series Classification
...
### 🥇 Leaderboard Metrics
Your models will be evaluated according to the **F1-Score**, computed as shown in the formula above, given the set of classes, the ground truth, and the model predictions.
...
- **Google Colaboratory** offers free cloud GPU access with usage limits.
- **Kaggle** also provides access to free GPUs, albeit with some limitations.
- For **local GPU** users, ensure that your device has the correct GPU drivers installed for your **CUDA** version.
